...
class GameAPI:
    ...
    def scrap_card(self, player: str, zone: str, index: int):
        self.game.scrap_card(player, zone, index)
    ...
    # Ally checks
    def faction_in_play(
        self, player: str, faction: str, min_count: int, scope: str
    ) -> bool:
...
class AbilityDispatcher:
    ...
    def activate_card(
        self, player: str, card: Dict[str, Any], ability_id: Optional[str] = None
    ):
        for ab in card.get("abilities", []):
    ...
    def _apply_effects(self, player: str, effects: List[Dict[str, Any]]):
        for e in effects:
            ...
            elif t in ("destroy_trade_row", "destroy_target_trade_row"):
                idx = self._pick_trade_row(e.get("filter"))
                if idx is not None:
